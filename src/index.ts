...
export class Codex {
  ...
  private registerControllers(controllers: readonly ControllerClass[]) {
    controllers.forEach((ControllerClass) => {
      ...
      const controllerMiddlewares: RequestHandler[] = Reflect.getMetadata('codex:middlewares', ControllerClass) || [];
      ...
      console.log(`✓ Registered controller: ${ControllerClass.name} at ${this.config.globalBaseRoute + basePath}`);
    });
...
type ProtectedRequest<ER extends ExtendedRequest<string, any> = ExtendedRequest<any, any>> = Request & {
